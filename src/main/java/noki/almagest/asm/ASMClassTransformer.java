...
public class ASMClassTransformer implements IClassTransformer, Opcodes {
	...
	class CustomMethodVisitor extends AnalyzerAdapter {
		...
		public void visitFieldInsn(int opcode, String owner, String name, String desc) {
			...
			if(opcode == GETFIELD
				...
				super.visitMethodInsn(INVOKESTATIC, "noki/almagest/asm/ASMClassTransformer", "renderLinesForVBO", "(F)V", false);
			}
				...
				callListFlag = true;
				super.visitVarInsn(FLOAD, 1);
				super.visitMethodInsn(INVOKESTATIC, "noki/almagest/asm/ASMClassTransformer", "renderLinesForCallList", "(F)V", false);
			}
	...
	class CustomMethodVisitor2 extends AnalyzerAdapter {
		...
		public void visitCode() {
			...
			super.visitMethodInsn(INVOKESTATIC, "noki/almagest/asm/ASMClassTransformer", "generateStarsAlt", "(Z)V", false);
...
//			super.visitEnd();
	...
	public static void renderLinesForVBO(float partialTicks) {
		...
		GameData data;
		for(int i=0; i<=88; i++) {
			data = AlmagestCore.savedDataManager.getFlagData().getConstellation(Constellation.getConstFromNumber(i).getName());
...
/*		double newX = vector.x*cosB*cosG - vector.y*cosB*sinG + vector.z*sinB;
	...
	public static void generateStarsAlt(boolean vboEnabled) {
